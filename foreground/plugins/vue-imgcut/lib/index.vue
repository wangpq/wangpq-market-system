<template>
    <div id="uppic">
        <slot></slot>
        <input class="file" accept="image/png,image/gif,image/jpeg" type="file" @change="getsrc" ref="file">
        <div id="uppicBox" ref="uppicBox" v-show="isuppicBox">
            <div class="imageBox">
                <img id="pic-img" ref="picImg" :src="img.src"
                     :style="{'transform':'translate3d('+transform.x+'px,'+transform.y+'px,0px) scale('+transform.scale+','+transform.scale+')','transition':'transform '+duration+'ms','width':img.w+'px','height':img.h+'px','margin-left':img.w/-2+'px','margin-top':img.h/-2+'px'}">
  ...
  export default {
    ...
    methods: {
      ...
      clip () {
        ...
        window.removeEventListener('mousewheel', this.zoomImg,false)
      ...
      zoomImg () {
        ...
        scale += event.wheelDelta / 600;//每次滚动加减0.2；
        if (scale > 0) {
          console.log("图片滚动缩放比例：" + scale + "%");
          this.transform['scale'] = scale;
        }
      }
    },
